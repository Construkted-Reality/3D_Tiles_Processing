...
async function crawlFolder(folderPath, outputFolderPath) {
    const files = await fs.readdir(folderPath, { withFileTypes: true });
    for (const file of files) {
        const fullPath = path.join(folderPath, file.name);
        if (file.isDirectory()) {
            await crawlFolder(fullPath, outputFolderPath);
        } else if (['.glb', '.b3dm'].includes(path.extname(file.name).toLowerCase())) {
            processFile(fullPath, outputFolderPath);
        }
    }
}
...
crawlFolder(inputFolderPath, outputFolderPath)
    .then(() => console.log('Processing complete.'))
    .catch(e => console.error('Error during folder crawling:', e));
