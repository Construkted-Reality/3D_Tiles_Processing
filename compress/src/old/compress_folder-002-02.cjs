...
if (isMainThread) {
    const MAX_WORKERS = Math.max(1, require('os').cpus().length - 1); // Use all cores except one
    let activeWorkers = 0;
    let fileQueue = [];
    let workerQueue = [];
    ...
    function processNextFile(worker) {
    ...
    processDirectory(directory, dracoCompression, ktx).then(() => {
        console.log(`Found ${fileQueue.length} files to process`);
        
        // Create workers and start processing
        const numWorkers = Math.min(MAX_WORKERS, fileQueue.length);
    ...
    parentPort.on('message', async (data) => {
        try {
            await compress(data.path, data.dracoCompression, data.ktx);
            parentPort.postMessage('done');
        } catch (error) {
            console.error(`Error processing file ${data.path}:`, error);
