...
// Define a silent logger
const silentLogger = {
    debug: () => {},
    warn: () => {},
    error: console.error  // Keep error logging for critical issues
};
...
async function compress(file, dracoCompression, ktx, doubleSided) {
    ...
    transforms.push(dedup({ 
        propertyTypes: [PropertyType.MATERIAL, PropertyType.MESH, PropertyType.ACCESSOR, PropertyType.TEXTURE],
        logger: {
        }
    }));
    ...
    if (file.endsWith(".b3dm")) {
        ...
        console.log('Statistics2:', statistics);

        await document.transform(...transforms, { logger: silentLogger }).catch(e => console.log(e));
